...
export class GradientRadial extends Gradient {    
    constructor(context: CanvasRenderingContext2D, width: number, height: number,         
        centerPosition: [number, number], innerRadius: number, outerRadius: number,
        gradientTransitionWidth: number) {

        super(context, gradientTransitionWidth);
        this.canvasGradient = this.context.createRadialGradient(width * centerPosition[0], height * centerPosition[1],
            innerRadius, width*centerPosition[0], height*centerPosition[1], outerRadius);
